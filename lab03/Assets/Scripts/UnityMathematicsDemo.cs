...
public class UnityMathematicsDemo : MonoBehaviour
{
    void Start()
    {
        ...
        UnityEngine.Debug.Log("Dot product of vectorA and vectorB: " + dotProduct); // Outputs: 32

        // math.cross: Calculates the cross product of two vectors
        float3 crossProduct = math.cross(vectorA, vectorB);
        UnityEngine.Debug.Log("Cross product of vectorA and vectorB: " + crossProduct); // Outputs: (-3, 6, -3)

        // math.normalize: Normalizes a vector (makes its magnitude equal to 1)
        float3 normalizedVector = math.normalize(vectorA);
        UnityEngine.Debug.Log("Normalized vectorA: " + normalizedVector); // Outputs a unit vector in the same direction as vectorA

        // math.distance: Calculates the distance between two points
        float distance = math.distance(vectorA, vectorB);
        UnityEngine.Debug.Log("Distance between vectorA and vectorB: " + distance); // Outputs the distance between vectorA and vectorB
        ...
        UnityEngine.Debug.Log("Clamped Value of 15 (0 to 10): " + clampedValue); // Outputs: 10
    }
}
