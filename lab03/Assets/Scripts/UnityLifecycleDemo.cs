...
public class UnityLifecycleDemo : MonoBehaviour
{
    void Awake()
    {
        UnityEngine.Debug.Log("Awake: Called once when the script instance is being loaded.");
    }

    void Start()
    {
        UnityEngine.Debug.Log("Start: Called before the first frame update, after all Awake calls.");
    }

    void FixedUpdate()
    {
        UnityEngine.Debug.Log("FixedUpdate: Called at a fixed time interval, used for physics-related updates.");
    }

    void Update()
    {
        UnityEngine.Debug.Log("Update: Called once per frame, used for regular updates like input handling.");
    }

    void LateUpdate()
    {
        UnityEngine.Debug.Log("LateUpdate: Called once per frame, after all Update calls, used for operations that depend on Update.");
    }
}
