...
public class RefOutExample : MonoBehaviour
{
    void Start()
    {
        // Using ref argument
        int refValue = 10;
        UnityEngine.Debug.Log("Initial refValue: " + refValue);
        ModifyValueUsingRef(ref refValue);
        UnityEngine.Debug.Log("Modified refValue: " + refValue); // Outputs: 20

        // Using out argument
        int outValue;
        ModifyValueUsingOut(out outValue);
        UnityEngine.Debug.Log("Modified outValue: " + outValue); // Outputs: 30

        // Without ref or out
        int normalValue = 10;
        UnityEngine.Debug.Log("Initial normalValue: " + normalValue);
        ModifyValueWithoutRefOrOut(normalValue);
        UnityEngine.Debug.Log("Modified normalValue: " + normalValue); // Outputs: 10 (no change)
    }
